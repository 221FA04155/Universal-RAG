...
async def signup(user: UserCreate):
    ...
    try:
        ...
        password_hash = await anyio.to_thread.run_sync(get_password_hash, password)
...
async def login(user_credentials: UserLogin, response: Response):
    ...
    try:
        ...
        # Verify password (CPU intensive, run in threadpool)
        start_verify = time.time()
        password_correct = await anyio.to_thread.run_sync(
            verify_password, password, user.password_hash
        )
        logger.info(f"Password verification took {time.time() - start_verify:.4f}s")
        ...
        # Set cookie (Adjusted for local/cross-origin testing)
        response.set_cookie(
            key="access_token",
            value=access_token,
            httponly=True,
            ...
            samesite="lax"  # Changed to lax for non-secure local dev
        )
